...
const toolFunctions: Record<string, ToolFunction> = {
  ...
  'binary-to-decimal': (input) =>
    isNaN(parseInt(input, 2)) ? '❌ Invalid binary' : parseInt(input, 2).toString(),
  'decimal-to-hex': (input) =>
    isNaN(Number(input)) ? '❌ Invalid decimal' : Number(input).toString(16),
  'hex-to-decimal': (input) =>
    isNaN(parseInt(input, 16)) ? '❌ Invalid hex' : parseInt(input, 16).toString(),
  'ascii-to-text': (input) =>
    input
      .split(' ')
  ...
  'number-to-words': (input) => {
    ...
    if (num < 1000) {
      ...
      return (
        ones[hundred] +
        ' hundred' +
        (rest ? ' ' + toolFunctions['number-to-words'](String(rest)) : '')
      );
    }
...
export default function PopularTools(data: any) {
  ...
  const selectedData = list.find((item) => (item.yrl ?? String(item.url_id)) === selectedTool);
  const selectedOption = toolOptions.find((opt) => opt.id === selectedTool);
  ...
  /* ---------- LOADING ---------- */
  if (loading) {
    return <div className="flex items-center justify-center h-screen">Loading tools...</div>;
  }
