...
const jsonToYaml = (obj: any, indent = 0): string => {
  ...
  if (Array.isArray(obj)) {
    return obj.map((item) => `${'  '.repeat(indent)}- ${jsonToYaml(item, indent + 1)}`).join('\n');
  }
...
const jsonToTable = (json: string, delimiter: ',' | '\t'): string => {
  ...
  const keys = Array.from(new Set(arr.flatMap((obj: any) => Object.keys(obj)))) as string[];
    ...
    switch (typeof value) {
      ...
      case 'object':
        if (Array.isArray(value)) {
          return value.length > 0 ? `List<${getJavaType(value[0])}>` : 'List<Object>';
        }
...
export function JsonConverters(data: any) {
  ...
  const selectedOption =
    list.find(
      (opt) =>
        (opt.route && opt.route === selectedConverter) || opt.id.toString() === selectedConverter
    ) || null;
  ...
  "email": "john@example.com"
              ...
              />
              <Button variant="link" onClick={handleExampleClick} className="mt-2 text-sm">
                Load Example
              </Button>
            </div>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 min-h-[300px] bg-gray-50 overflow-auto">
                {outputText ? (
                  outputText.startsWith('❌') ? (
                    <p className="text-red-500 whitespace-pre-wrap break-words">{outputText}</p>
                  ) : (
